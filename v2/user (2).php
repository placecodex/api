...
class User extends CI_Controller {
	...
	function signup(){
		...
		$query = $this->db->query("SELECT username, password FROM users_rest WHERE username='{$username}'");
		if ($query->num_rows > 0) {
			...
			$this->db->insert('users_rest', $data); 
			$query = $this->db->query("SELECT id FROM users_rest WHERE username='{$username}'");
